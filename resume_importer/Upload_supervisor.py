...
class UploadSupervisor(object):
    ...
    def __init__(self, cml_args, files):
        ...
        self.client = hf.Client(api_secret=cml_args.api_key, api_url=cml_args.api_url)
        self.paths = files
    ...
    def _init_workers(self):
        for i in range(self.n_worker):
            self.workers[i] = Upload_worker.Upload_worker(i, self.client, self.source_id, self.timestamp_reception, self.can_move_to_fail_folder, self.fail_folder_path)
